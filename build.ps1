<#
.SYNOPSIS
    CipherTron buildScript
.DESCRIPTION
    A custom Psake buildScript for the module CipherTron.
.LINK
    https://github.com/alainQtec/CipherTron/blob/main/build.ps1
.EXAMPLE
...
Begin {
    ...
    $dataFile = [System.IO.FileInfo]::new([IO.Path]::Combine($PSScriptRoot, "$([System.Globalization.CultureInfo]::InstalledUICulture[0].Name)", 'CipherTron.strings.psd1'))
            ...
            Task Deploy -depends Test -description 'Release new github version and Publish module to PSGallery' {
                if ([Environment]::GetEnvironmentVariable($env:RUN_ID + 'BuildSystem') -eq 'VSTS' -or ($env:CI -eq "true" -and $env:GITHUB_RUN_ID)) {
                    ...
                    try {
                        ...
                        $latest_Github_release = Invoke-WebRequest "https://api.github.com/repos/alainQtec/CipherTron/releases/latest" | ConvertFrom-Json
